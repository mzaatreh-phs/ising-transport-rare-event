...
# ============================================================
def fig_prefix():
    # (pi_hat, rel_sd, cost, fom, fom_ratio_vs_WEm, zero_rep) — matches the
    # retired Table~\ref{tab:prefix} exactly; this figure is now the sole
    # carrier of that data (table removed, data folded in here instead).
    ferro = {
        "naive":            (9.52e-6, 0.441, 1.34e8, 3.83e-8, 2.92, 0),
        "WE[m]":            (1.05e-5, 0.614, 2.02e8, 1.31e-8, 1.00, 0),
        r"WE[$E$]":         (1.09e-5, 1.343, 9.81e7, 5.65e-9, 0.43, 2),
        r"WE[$I_\theta$]":  (7.92e-6, 0.893, 1.82e8, 6.90e-9, 0.53, 0),
    }
    ea = {
        "naive":            (1.00e-5, 0.633, 1.34e8, 1.86e-8, 2.38, 1),
        "WE[m]":            (1.02e-5, 1.857, 3.72e7, 7.80e-9, 1.00, 7),
        r"WE[$E$]":         (1.65e-5, 1.115, 1.61e8, 5.00e-9, 0.64, 2),
        r"WE[$I_\theta$]":  (1.54e-5, 1.052, 1.42e8, 6.36e-9, 0.81, 2),
    }

    fig = plt.figure(figsize=(6.6, 5.6))
    gs = fig.add_gridspec(2, 2, height_ratios=[2.3, 1.6], hspace=0.55, wspace=0.08)
    axes = [fig.add_subplot(gs[0, 0]), fig.add_subplot(gs[0, 1])]
    axes[1].sharey(axes[0])

    for ax, data, title in zip(axes, [ferro, ea], ["ferromagnet", "EA spin glass"]):
        methods = list(data.keys())
        fom = [data[m][3] for m in methods]
        zeros = [data[m][5] for m in methods]
    ...
    fig.suptitle(r"Full scale ($L=16$, 10 replicas), target one step beyond "
                 "the pilot's observed extreme", fontsize=9, y=0.98)

    # ---- underlying data table (was Table~\ref{tab:prefix}) ----
    ax_tab = fig.add_subplot(gs[1, :])
    ax_tab.axis("off")
    col_labels = ["Method", r"$\hat\pi$", "rel. s.d.", "cost",
                  r"FOM (vs. WE[$m$])", "zero-rep."]
    rows, row_colors = [], []
    for group_name, data in [("Ferromagnet", ferro),
                              ("EA spin glass", ea)]:
        rows.append([group_name, "", "", "", "", ""])
        row_colors.append("#eeede7")
        for m, (pi, rsd, cost, fom, ratio, z) in data.items():
            m_plain = m.replace("$", "").replace(r"\theta", "θ").replace(
                r"WE[", "WE[").replace("[E]", "[E]")
            rows.append([m_plain, f"{pi:.2e}", f"{rsd:.3f}", f"{cost:.2e}",
                         f"{fom:.2e} ({ratio:.2f}×)", f"{z}/10"])
            row_colors.append("white")
    tab = ax_tab.table(cellText=rows, colLabels=col_labels, cellLoc="center",
                        loc="center", bbox=[0.0, 0.0, 1.0, 1.0])
    tab.auto_set_font_size(False)
    tab.set_fontsize(6.3)
