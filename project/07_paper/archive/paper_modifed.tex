...
\subsubsection{Scope of the correspondence}\label{sec:scope}
...
committor gave measured relative variance $\sim10^{-30}$ (zero to machine precision), a
FOM improvement over naive Monte Carlo of many orders of magnitude, exactly as
Eq.~\eqref{eq:committor}'s zero-variance property predicts. Tilting by an
\emph{approximate} committor under pure biasing, with no weight-window control on the
resulting weights, instead performed \emph{worse} than plain naive Monte Carlo. This is
the reason every subsequent learned-coordinate result in this paper uses Weighted
Ensemble (split/roulette) rather than raw importance reweighting: an imperfect importance
estimate, used to reweight without bounding the weights, can be actively harmful.
...
weight windows to the same \emph{approximate}, otherwise-fragile committor tilt described
above recovered roughly a $2\times$ figure-of-merit improvement over the same tilt without
them (measured FOM $1.24\times10^{-5}\to4.86\times10^{-4}$):
...
depth (naive itself now has 4/10 zero-replicas, versus 1/10 at the shallower target), so
the numbers are noise over noise, not a result. This is the run that established the
ladder-difficulty discipline of \S\ref{sec:methods} for the remainder of the project.
This rarity is stretching every method's budget, not only the weight-window ones, so a
trustworthy comparison here needs a different fix than anything tried at the shallower
depth. This motivated the milestoning objective below, which is a change to the training
label rather than to the weight-window schedule; Table~\ref{tab:milestone2} in
\S\ref{sec:milestone} reproduces this exact run alongside its milestoning-based
resolution at the same depth, side by side.
...
The same nominal depth was reached two different ways, shown side by side below: the
surrogate-labeled network from \S\ref{sec:res-learned}, where the comparison was noise
over noise, and the milestoning-labeled network, which reaches this depth cleanly
(0/10 zero-replicas throughout) but still does not favor $\mathrm{WE}[I_\theta]$.

% --- TABLE 4/6 (merged) ---
\begin{table}[H]
\centering\small
\caption{EA spin glass, $L=16$, one threshold step rarer than the shallower baseline in
	each row-group, reached two different ways. Top: surrogate-labeled network (same
	training as Table~\ref{tab:prefix}), one step rarer than Table~\ref{tab:prefix} --- not
	a trustworthy comparison, shown for reference only (see text). Bottom: milestoning
	labels (same training as Table~\ref{tab:milestone}), one step rarer than
	Table~\ref{tab:milestone}, default configuration ($n_{\rm bins}=50$, $R=10$) --- a
	clean regime.}
\label{tab:beyond2}
...
\begin{tabular}{lrr}
	...
	Method & FOM & zero-rep.\ \\
	\midrule
	\multicolumn{3}{l}{\emph{Surrogate labels, one step rarer than Table~\ref{tab:prefix} --- not trustworthy}} \\
	naive & $7.43\times10^{-9}$ & 4/10 \\
	$\mathrm{WE}[m]$ & $0$ & 10/10 \\
	$\mathrm{WE}[E]$ & $2.18\times10^{-9}$ & 5/10 \\
	$\mathrm{WE}[I_\theta]$ & $1.56\times10^{-9}$ & 7/10 \\
	\multicolumn{3}{l}{\emph{Milestoning labels, one step rarer than Table~\ref{tab:milestone} --- clean regime}} \\
