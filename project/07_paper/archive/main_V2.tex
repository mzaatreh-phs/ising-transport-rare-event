...
% Short title
\shorttitle{From Radiation Shielding to Spin Glasses: Auditing the Adjoint-Importance Toolkit for Rare-Event Sampling}
...
\item Deep-ensemble follow-up resolves that ambiguity: a decisive $0.087\times$ loss (95\% CI $[0.051,0.155]$), the paper's only interval excluding parity.
...
checkpoints, and the CI cannot be closed by seed count alone
...
$+112\%$ spread in Table~\ref{tab:milestone}. But the value it converges toward is a loss,
not the earlier
...
Figure~\ref{fig:ensemble} shows both halves of that verdict together, the variance
collapsing as intended and the centre of the distribution settling below parity.

% --- FIGURE: deep-ensemble resolution ---
\noindent
\begin{minipage}{\linewidth}
\centering
\includegraphics[width=0.92\textwidth]{figs/fig_ensemble.pdf}
\captionof{figure}{Deep ensembling does what it was built to do to the variance, and
	converges to a loss. Left: per-run FOM ratios against $\mathrm{WE}[E]$, the 17
	single-net seeds of Table~\ref{tab:milestone} against the five $K{=}5$ ensembles of
	Table~\ref{tab:ensemble}; the spread narrows from $0.48$--$2.12\times$ to
	$0.46$--$0.91\times$, and every ensemble run lands below parity. Right: the three
	pooled two-level-bootstrap estimates. The single-net pool sits above 1 and the
	ensemble pool below it, both with intervals crossing parity; once the ensemble's real
	inference cost is counted at all (wall-clock rather than MC-sweep cost), the interval
	clears parity outright. This is the only confidence interval in the paper that does.}
\label{fig:ensemble}
\end{minipage}
...
coordinate and this project's own surrogate coordinate: FOM $1.62\times10^{-8}$ for
self-consistency against $1.33\times10^{-8}$ for $\mathrm{WE}[E]$ and $8.62\times10^{-9}$
for the surrogate objective (naive and $\mathrm{WE}[m]$ reach $8.60\times10^{-8}$ and
$8.81\times10^{-8}$ at this shallow, still naive-favorable depth).
...
$1.19\times$ point estimate (Table~\ref{tab:milestone}) instead settled it in the negative,
with a 95\% CI
($0.087\times$, $[0.051,0.155]$, on a corrected cost accounting) that excludes
parity, the only bootstrap interval in this paper that does (Figure~\ref{fig:ensemble}).
...
outcomes from $-52\%$ to $+112\%$ (Table~\ref{tab:milestone}), and pooling them left a
