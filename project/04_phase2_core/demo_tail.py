"""demo_tail.py -- monotone one-sided rare tail P(|m| >= m*) above Tc.
The cleanest deep-penetration analog: no barrier-crossing dynamics, just an
importance gradient rising into the tail. WE resolves the whole tail with
uniform relative error; naive MC (matched cost) dies once |m| is large."""
import time, numpy as np
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
from ising_we import (weighted_ensemble, sweep_population, magnetization,
                      exact_Pm, TC)
...
def main():
    # ---- L=4 gate on |m| (fold exact P(m)) ----
    me, Pe = exact_Pm(4, 2.6)
    Pabs = {}
    for mm, p in zip(me, Pe):
        Pabs[abs(round(mm, 6))] = Pabs.get(abs(round(mm, 6)), 0) + p
    cg, Pg, _ = weighted_ensemble(4, 2.6, n_bins=9, n_per_bin=20, tau=3,
                                  n_iter=2500, burn=500, seed=3,
                                  coord=absm, crange=(0, 1))
    err = [abs(np.log(Pg[int(np.argmin(np.abs(cg-k)))]+1e-30)-np.log(v))
           for k, v in Pabs.items() if v > 1e-9]
    print(f"[gate L=4] mean |dlnP(|m|)| = {np.mean(err):.3f}  (unbiased if small)\n")

    # ---- demonstration ----
    L, T, n_bins, R = 20, 2.60, 22, 4
    ...
    def stats(P):
        ...
        return mean, rel
    mW, relW = stats(Pw); mN, relN = stats(Pn)
    N = L * L
    ...
    # save data so the figure can be regenerated without recomputing
    np.savez("tail_data.npz", centers=centers, mW=mW, relW=relW, mN=mN,
             relN=relN, L=L, T=T, N=N, we_cost=we_cost)

    okW, okN = mW > 0, mN > 0
    naive_floor = centers[okN].max()          # deepest |m| naive ever reaches
    we_floor = centers[okW].max()

    fig, ax = plt.subplots(1, 2, figsize=(10.6, 4.1))
    ax[0].plot(centers[okW], -np.log(mW[okW]) / N, "o-", color=BLUE, ms=4,
               label="Weighted Ensemble")
    ax[0].plot(centers[okN], -np.log(mN[okN]) / N, "s", color=RED, ms=6,
               label="naive MC (matched cost)")
    ...
    ax[0].set_title("(a) WE reaches ~2 decades deeper\nthan naive's hard support floor")
    ax[0].legend(frameon=False, fontsize=9); ax[0].grid(alpha=0.3)

    ax[1].semilogy(centers[okW], relW[okW], "o-", color=BLUE, ms=4,
                   label="Weighted Ensemble")
    ax[1].semilogy(centers[okN], relN[okN], "s-", color=RED, ms=6, label="naive MC")
    ...
    fig.tight_layout(); fig.savefig("figs/ising_tail.pdf", bbox_inches="tight")
    print(f"\nnaive floor |m|={naive_floor:.3f} (P~{mN[okN][-1]:.1e}); "
          f"WE reaches |m|={we_floor:.3f} (P~{mW[okW][-1]:.1e})")
    print(f"saved figs/ising_tail.pdf  [{time.time()-t0:.0f}s]")
